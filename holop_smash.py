...
class Smasher:
    ...
    async def handle_bomb(self, mined):
        ...
        # 💰 ЗАРАНЕЕ (ДО разминирования) снимаем серебро на лечение — так велели Владимир и
        # Ксюша: пока бот разминирует (~20-30с), игра успевает «увидеть» снятые деньги, и если
        # рванёт — лечение проходит сразу. Иначе десинк: деньги сняты, а игра их «не видит».
        if self._bomb_defense or self._defense_only:
            try:
                await self._ensure_silver_for_heal()
            except Exception as e:
                if _is_dead_session(e):
                    raise
                self._bomb_log(f"  ⚠️ пред-снятие серебра сорвалось: {type(e).__name__}")
            mined = await self.refetch(mined.id) or mined   # вернулись из казны — освежаем бочку
