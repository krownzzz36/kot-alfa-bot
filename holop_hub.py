...
VERSION = "2026.07.29-52"   # видно в консоли и в шапке панели — чтобы понимать, свежая ли версия
...
MODULES = [
    ...
    {
        "id": "guard", "title": "Безопасный режим", "emoji": "🔒", "kind": "guard",
        "desc": "Не фармит — только оберегает: разминирует бочки, держит охрану на холопах, отбивает кражу холопа, казна, оборона.",
    },
    {
...
function render(mid){
  ...
  if(m.kind==='guard'){
    ctl=`<details class="acc" open>
        <summary><span class="acc-ic">🔒</span><span class="acc-t">Безопасный режим</span><span class="chev"></span></summary>
        <div class="acc-body">
          <div class="note">Кот <b>НЕ фармит</b> (не атакует, максимально не палевно), а только <b>оберегает</b>: разминирует бочки (Огниво → красный фитиль) и восстанавливает после взрыва, <b>держит охрану на холопах</b>, <b>мгновенно отбивает кражу холопа</b> (выкуп → защита), собирает казну и реинвестит, держит ров/частокол. Ниже — что включить. <b>Огниво держи в запасе (10–20 шт.)</b> — покупку бот не делает.</div>
          <div class="grid2" style="margin-top:12px">
            ${swHTML('set_defense_only','🔒 Только оберегать, не фармить (спокойный режим)')}
            ${swHTML('set_bomb_defense','💣 Разминировать бочки + восстановление после взрыва')}
            ${swHTML('set_auto_guard','🛡️ Держать охрану на холопах (авто-защита)')}
            ${swHTML('set_holop_guard','🚨 Анти-кража холопа (украли → выкупаю обратно и защищаю)')}
            ${swHTML('set_auto_kazna','🏦 Авто-казна (сбор → депозит → реинвест)')}
            ${swHTML('set_auto_defense','🧱 Авто-оборона (ров + частокол активны + запас)')}
          </div>
          <div class="btns" style="margin-top:12px">
            <button id="btnStart" class="b-green" onclick="startGuard()">▶ Запустить безопасный режим</button>
            <button id="btnStop" class="b-red" onclick="post('raids','stop')">⏹ Остановить</button></div>
          <div class="note" id="gnote">Тот же кот, что и «Набеги». «Запустить безопасный режим» включит режим «только оберегать» и все галочки безопасности выше. Анти-кража и разминирование работают и в бою тоже.</div>
        </div>
      </details>`;
  } else if(m.kind==='loop'){
    ctl=`<details class="acc" open>
...
const GUARD_SW=['set_defense_only','set_bomb_defense','set_auto_guard','set_holop_guard','set_auto_kazna','set_auto_defense'];
async function saveGuard(){ const body={}; GUARD_SW.forEach(id=>{const e=$('#'+id); if(e) body[id.slice(4)]=!!e.checked;}); try{ await fetch('/api/raids/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); const n=$('#gnote'); if(n) n.textContent='Сохранено ✓ — меняется на лету.'; }catch(e){} }
async function startGuard(){ ['set_defense_only','set_bomb_defense','set_auto_guard','set_holop_guard'].forEach(id=>{const e=$('#'+id); if(e) e.checked=true;}); await saveGuard(); post('raids','start'); }
async function loadGuard(){ try{ const d=await (await fetch('/api/raids/settings')).json(); const on={set_defense_only:!!d.defense_only,set_bomb_defense:d.bomb_defense!==false,set_auto_guard:!!d.auto_guard,set_holop_guard:d.holop_guard!==false,set_auto_kazna:!!d.auto_kazna,set_auto_defense:!!d.auto_defense}; GUARD_SW.forEach(id=>{const e=$('#'+id); if(e){ e.checked=!!on[id]; e.onchange=saveGuard; }}); }catch(e){} }
...
const GUIDE_SECTIONS=[
 ...
 ['🎛️','Набеги — Настройки боя',`
   ...
   <p><b>Два режима бота.</b> Все безопасные (неатакующие) действия — казна, охрана холопов, оборона, разминирование бочек, анти-кража холопа — работают в ОБОИХ режимах: и в «Набегах» (боевой), и во вкладке «🔒 Безопасный режим» (спокойный, без атак). Разница только в том, что боевой ещё и бьёт по целям. Так что можно оставить бота в безопасном режиме на ночь — он не фармит, но казну собирает, охрану держит, бочки гасит и отбивает кражу холопов.</p>
